...
class DepthScanner(object):
    def __init__(self, camera:int=0, mode:str="live", scale:float=1, color:str="hot"):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, display_frame, label=label, color=colors[int(cls)], line_thickness=1)
            ...
            if view_img:
                ...
                cv2.imshow("testsetset", display_frame)
                ...
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        if vid_cap:  # video
                            ...
                            save_path += '.mp4'
                        vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (int(w*2), h))
                    # 
                    concat_frame = np.concatenate((im0, display_frame), axis=1)
                    vid_writer.write(concat_frame)
...
if __name__ == '__main__':
    ...
    DepthScanner_class = DepthScanner()
    with torch.no_grad():
